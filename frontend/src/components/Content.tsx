    ...
    <>
      ...
      <div className={`n-bg-palette-neutral-bg-default ${classNameCheck}`}>
        <Flex className='w-full' alignItems='center' justifyContent='space-between' flexDirection='row'>
          ...
          />
          <div className='connectionstatus__container'>            <span className='h6 px-1'>Neo4j 连接状态</span>
            <Typography variant='body-medium'>
              ...
              {connectionStatus ? (
                ...
                <span className='n-body-small'>未连接</span>
              )}
              <div className='pt-1'>
                {!isSchema ? (
                ...
                {isSchema ? (                  <span className='n-body-small'>
                    {(!selectedNodes.length || !selectedNodes.length) && '空'} 图谱模式已配置
                    {selectedNodes.length || selectedRels.length
                      ? `(${selectedNodes.length} 标签 + ${selectedRels.length} 关系类型)`
                      : ''}
                  </span>
                ) : (
                  <span className='n-body-small'>未配置图谱Schema模式</span>
                )}
              </div>
            </Typography>
          </div>
          <div>            <Button className='mr-2.5' onClick={openGraphEnhancementDialog} disabled={!connectionStatus}>
              图谱增强
            </Button>
            {!connectionStatus ? (
        ...
        >          <DropdownComponent
          ...
          <Flex flexDirection='row' gap='4' className='self-end'>
            <ButtonWithToolTip              text={tooltips.generateGraph}
              placement='top'
              label='生成图谱'
            ...
            <ButtonWithToolTip
              ...
              disabled={!filesData.some((f) => f?.status === 'Completed')}              className='ml-0.5'
              label='使用Bloom打开图谱'
            >
