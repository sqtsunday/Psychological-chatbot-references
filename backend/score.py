...
# 将两个目录都添加到 Python 路径
sys.path.insert(0, project_root)  # 项目根目录（用于 backend.src.xxx 导入）
sys.path.insert(0, current_dir)   # backend 目录（用于 src.xxx 导入）
...
# # 找到第21行左右的 from volcenginesdkarkruntime import Ark
# # 将其替换为:
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, port=int(os.environ.get('TCP_PORT', '8080')))